...
class Customer:
    ...
    def customer_details(self):
        print(
            f"Customer {self.identifier} with basket size {self.__basket_size} | Checkout Time: {4 * self.__basket_size} seconds. "
            f"| Lottery Ticket : {' - Lottery Winner!' if self.__lottery_ticket else ' '}")
...
class LaneManager:
    ...
    def open_lanes(self):
        lane1.lane_open() # Sets regular lane 1 to open at start
        ss1.lane_open() # Sets Self Service lane 1 to open at start
        lanes = [lane1, lane2, lane3, lane4, lane5] # placed al regular lanes in list to manage it easily.
...
class SupermarketSimulation:
    ...
    def simulate_checkout_and_close_lanes(self):
        # Simulate checkout and close lanes
        self.simulate_checkout()
    ...
    def generate_random_customers(self):
        # Generates a random number of customers and assigns them to checkout lanes
    ...
    def stop_simulation(self):
        self.stop_event.set()
        print("\nSending the request to stop the simulation. Please wait few seconds. Thanks!")
        ...
        try:
            ...
            while current_time - start_time < total_simulation_duration:
                ...
                if current_time - start_time < customer_generation_duration:
                    ...
                    # Simulate checkout and close lanes in a separate thread
                    checkout_thread = threading.Thread(target=self.simulate_checkout_and_close_lanes)
                    checkout_thread.start()
